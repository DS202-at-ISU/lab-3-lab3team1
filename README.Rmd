...
av = av %>% 
  mutate(Time = 0)
for(row in 1:nrow(av)){
  av$Time[row] = sum(av[row,c('Death1', 'Death2', 'Death3', 'Death4', 'Death5')] == 'YES')
}
...
```{r}
av <- av %>% rowwise() %>% mutate(Deaths = sum(c(Return1, Return2, Return3, Return4, Return5) == "YES"))
av <- av %>% rowwise() %>% mutate(Returns = sum(c(Return1, Return2, Return3, Return4, Return5) == "YES"))
```
